...
def main():
    ...
    lemma_csv = os.path.join(args.out_dir, "serbian_verbs_frequency_hf.csv")
    save_lemma_freq(lemma_counter, lemma_csv)

    pairs = build_pairs_from_freq(lemma_counter)
    pairs_csv = os.path.join(args.out_dir, "serbian_verb_pairs_hf.csv")
    save_pairs(pairs, pairs_csv)

    pairs_500 = filter_pairs_min_freq(pairs, args.min_freq)
    final_csv = os.path.join(
        args.out_dir, "serbian_verb_final_hf_wiki_500.csv"
    )
